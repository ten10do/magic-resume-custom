...
const BaseInfo = ({ basic = {} as BasicInfo, globalSettings, template }: BaseInfoProps) => {
    ...
    return (
        <SectionWrapper sectionId="basic">
            <div className={styles.container} style={isTopRightPhoto ? { position: "relative", minHeight: `${basic.photoConfig?.height || 100}px`, alignItems: "stretch" } : undefined}>
                <div className={styles.header} style={{ flexDirection: isTopRightPhoto ? "column" : photoPosition === "top" ? "column" : photoPosition === "right" ? "row-reverse" : "row", ...(isTopRightPhoto ? { width: "100%", alignItems: "stretch" } : {}) }}>
                    {PhotoComponent}
                    <div className={`flex flex-col ${styles.nameTitle}`} style={{ color: "#fff", alignItems: alignmentItems[nameAlignment], textAlign: nameAlignment, ...(isTopRightPhoto ? { width: "100%", maxWidth: "none" } : {}) }}>
                        ...
                        {titleField.visible !== false && basic[titleField.key] && (
                            <motion.h2 layout="position" className="mt-1 whitespace-normal break-normal [overflow-wrap:normal]" style={{ fontSize: `${basic.titleFontSize || 18}px`, color: "#fff" }}>{basic[titleField.key] as string}</motion.h2>
                        )}
                    </div>
                </div>
                <motion.div layout="position" className={styles.fields}
                    style={{ fontSize: `${basic.contactFontSize || globalSettings?.baseFontSize || 14}px`, color: "#fff", alignItems: alignmentItems[contactAlignment], textAlign: contactAlignment }}>
                    {allFields.map((item) => {
                        const customFieldHref = item.custom && "href" in item && typeof item.href === "string" ? item.href : null;

                        return (
                        <motion.div key={item.key} className="flex min-w-0 items-start text-baseFont" style={{ width: "100%", color: "#fff", justifyContent: contactAlignment === "left" ? "flex-start" : contactAlignment === "right" ? "flex-end" : "center" }}>
                            {useIconMode ? (
